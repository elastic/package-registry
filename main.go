...
func getRouter(logger *zap.Logger, config *Config, indexer Indexer) (*mux.Router, error) {
	...
	detectionRulesHandler := detectionRulesHandlerWithProxyMode(logger, indexer, proxyMode, config.CacheTimeSearch)
	...
	router.HandleFunc("/detection_rules", detectionRulesHandler)
