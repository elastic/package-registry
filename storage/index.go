...
func loadSearchIndexAllFunc(ctx context.Context, logger *zap.Logger, storageClient *storage.Client, bucketName, rootStoragePath string, aCursor cursor, process func(ctx context.Context, p *packageIndex) error) error {
	span, ctx := apm.StartSpan(ctx, "LoadSearchIndexAll", "app")
	defer span.End()
